...
list_of_line = list()
list_of_email = list()
# storing Input lines in a list
for _ in range(num_of_line):
    list_of_line.append(input())
# Regex to split and store all email type word in a line
Regex_pattern_line = re.compile(r'(\w+(\.\w+)*@\w+(\.\w+)*)')
for line in list_of_line:
    # Regex to validate whether a word is correct email or not
    Regex_pattern_email = re.compile(r'\w+(\.\w+)*@\w+(\.\w+)*')
    splitted_lines = re.split(Regex_pattern_line, line)
    for sub_line in splitted_lines:
        if sub_line:
            match = re.search(Regex_pattern_email, sub_line)
            if match:
                list_of_email.append(str(match.group(0)))
print(';'.join(sorted(set(list_of_email))))
