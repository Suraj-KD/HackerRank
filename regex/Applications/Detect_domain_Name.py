...
Regex_pattern_domain = re.compile(r'https?://(www2?\.)?([a-zA-Z0-9\-]+(\.[a-zA-Z0-9\-]+)+)')
for i in list_of_lines:
    founded = re.findall(Regex_pattern_domain, i)
    for f in founded:
        list_of_domain.append(f[1])
print(';'.join(sorted(set(list_of_domain))))
