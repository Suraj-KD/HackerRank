...
IPv4 was the first publicly used Internet Protocol which used 4 byte addresses which permitted for 2^32 addresses. The typical format of an IPv4 address is A.B.C.D where A, B, C and D are Integers lying between 0 and 255 (both inclusive).
...
'''
import re, sys
Regex_ipv4 = r'^(\d{1,4}\.){3}\d{1,4}$'
Regex_ipv6 = r'^(([0-9a-f]{1,4}):){7}([0-9a-f]{1,4})$'
for i in sys.stdin.readlines()[1:]:
    if re.search(Regex_ipv4, i):
        num_list = re.split('\.', i)
        ipv4_flag = True
        for num in num_list:
            if not ((0 <= int(num)) and (int(num) <= 255)):
                ipv4_flag = False
        if ipv4_flag:
            print("IPv4")
        else:
            print("Neither")
    elif re.search(Regex_ipv6, i):
        print("IPv6")
    else:
        print("Neither")
